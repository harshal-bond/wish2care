...
export function computeBmiCategory(bmi) {
    if (bmi == null)
        return null;
    if (bmi < 18.5)
        return 'Underweight';
 ...
 */
export function computeGrowthScore(bmiCategory, muac) {
    if (!bmiCategory)
        return null;
...
function lookupPoints(field, value) {
    if (isBlank(value))
        return null;
    const table = SCORING_POINTS[field];
    const pts = table[value];
    return pts === undefined ? null : pts;
}
/** ROUND(AVERAGE(points) * 20, 0); blank if any response missing */
export function averageDomainScore(points) {
    if (points.length === 0 || points.some((p) => p == null))
        return null;
    const avg = points.reduce((a, b) => a + b, 0) / points.length;
    return Math.round(avg * 20);
}
export function computeDietScore(input) {
...
export function computeLifestyleScore(input) {
    return averageDomainScore([
...
export function computeMedicalHistoryScore(input) {
    return averageDomainScore([
...
export function computeMentalWellnessScore(input) {
    return averageDomainScore([
...
export function computeOverallHealthScore(parts) {
    ...
    return Math.round(growth * DOMAIN_WEIGHTS.growthAnthropometry +
...
export function computeNutritionScore(growth, diet) {
    if (growth == null || diet == null)
        return null;
 ...
 */
export function computeUndernutritionRiskScore(nutritionScore, weightLoss, poorAppetite, muac) {
...
export function computeRiskCategory(overall) {
    if (overall == null)
        return null;
    if (overall >= 90)
        return 'Green - Healthy';
    if (overall >= 75)
        return 'Light Green - Mild Watch';
    if (overall >= 60)
...
export function computeNeedReferral(overall, undernutritionRisk, chronicDisease) {
    if (overall == null)
        return null;
...
export function computeNeedDoctorReview(riskCategory, needReferral) {
    if (riskCategory == null || needReferral == null)
        return null;
    if (riskCategory === 'Orange - Moderate Risk' ||
...
export function computeScreeningScores(input) {
    ...
    const undernutritionRiskScore = computeUndernutritionRiskScore(nutritionScore, input.weightLoss, input.poorAppetite, input.muac);
