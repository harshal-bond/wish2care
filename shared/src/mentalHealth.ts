...
export const MH_REVERSE_ITEM_INDICES = [3, 7, 17, 21, 28] as const;

export const MH_REVERSE_QUESTION_NUMBERS = MH_REVERSE_ITEM_INDICES.map((i) => i + 1);

export function reverseLikertScore(
  original: number,
  scaleMax: number = MH_SCALE_MAX
): number {
  return scaleMax + 1 - original;
}

export function isReverseCodedItem(zeroBasedIndex: number): boolean {
  return (MH_REVERSE_ITEM_INDICES as readonly number[]).includes(zeroBasedIndex);
}
...
export function normalizeItemScore(
  ...
  if (original == null || Number.isNaN(Number(original))) return null;
...
export function computeNormalizedMhTotal(
