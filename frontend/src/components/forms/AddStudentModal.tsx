...
export function AddStudentModal({ 
  ...
  const defaultSchoolId = user?.role === 'fieldworker' && user?.assignedSchoolId ? user.assignedSchoolId : undefined;
      ...
      >
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Enter complete student details matching the bulk upload format.</p>
          </div>
        ...
        <div className="overflow-y-auto p-6 flex-1">
          <form id="add-student-form" onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Required Information</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" {...register('age', { valueAsNumber: true })} placeholder="e.g. 14" className="rounded-xl h-11" />
                  {errors.age && <p className="text-xs text-red-500">{errors.age.message}</p>}
                </div>

                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Gender <span className="text-red-500">*</span></label>
                  <select {...register('gender')} className="flex h-11 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-950">
                ...
                <div className="space-y-2 lg:col-span-4">
                  <label className="text-sm font-semibold text-gray-700">School <span className="text-red-500">*</span></label>
                  <Controller
                    name="schoolId"
                    control={control}
                    render={({ field }) => (
            ...
            <div className="space-y-4 pt-4 border-t border-gray-100">
              <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Contact & Additional Details</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Student Code</label>
                  <Input {...register('studentCode')} placeholder="Auto-generated if empty" className="rounded-xl h-11" />
                  {errors.studentCode && <p className="text-xs text-red-500">{errors.studentCode.message}</p>}
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Date of Birth</label>
                  <Input {...register('dateOfBirth')} placeholder="DD-MM-YYYY or similar" className="rounded-xl h-11" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Blood Group</label>
                  <Input {...register('bloodGroup')} placeholder="e.g. O+" className="rounded-xl h-11" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Email</label>
                  <Input type="email" {...register('email')} placeholder="student@example.com" className="rounded-xl h-11" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Mobile No.</label>
                  <Input {...register('mobileNo')} placeholder="e.g. 9876543210" className="rounded-xl h-11" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Father's Mobile No.</label>
                  <Input {...register('fatherMobileNo')} placeholder="e.g. 9876543210" className="rounded-xl h-11" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Nominee Name</label>
                  <Input {...register('nomineeName')} placeholder="e.g. Sunita Devi" className="rounded-xl h-11" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Relationship</label>
                  <Input {...register('relationship')} placeholder="e.g. Mother" className="rounded-xl h-11" />
                </div>
              </div>
            </div>
            ...
            {/* Academic & Address */}
            <div className="space-y-4 pt-4 border-t border-gray-100">
              <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Academic & Address</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Course Name</label>
                  <Input {...register('courseName')} placeholder="e.g. Class 10" className="rounded-xl h-11" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">College Stream</label>
                  <Input {...register('collegeStream')} placeholder="e.g. Science" className="rounded-xl h-11" />
                </div>
                <div className="space-y-2 md:col-span-2">
                  <label className="text-sm font-semibold text-gray-700">Local Address</label>
                  <Input {...register('localAddress')} placeholder="Enter full address" className="rounded-xl h-11" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Area</label>
                  <Input {...register('area')} placeholder="e.g. North Zone" className="rounded-xl h-11" />
                </div>
              </div>
            </div>
          </form>
        ...
        <div className="p-6 border-t border-gray-100 flex gap-4 bg-gray-50/50 justify-end">
          ...
          >
            {addMutation.isPending ? <Loader2 className="h-5 w-5 animate-spin mr-2" /> : null}
