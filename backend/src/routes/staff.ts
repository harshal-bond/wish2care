...
staffRoutes.get('/', async (c) => {
  ...
  if (search) {
    ...
    conditions.push(
      or(ilike(staff.name, searchPattern), ilike(staff.staffCode, searchPattern))
    );
  }
...
staffRoutes.get('/:id', async (c) => {
  ...
  if (rows.length === 0) return c.json({ success: false, error: 'Staff not found' }, 404);
...
staffRoutes.post('/', async (c) => {
  try {
    ...
    const [created] = await db.insert(staff).values(result.data as any).returning();
...
staffRoutes.put('/:id/assessment', async (c) => {
  ...
  if (isNaN(staffId)) return c.json({ success: false, error: 'Invalid ID' }, 400);
