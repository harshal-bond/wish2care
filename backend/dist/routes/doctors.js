...
function generateSlotsForDay(startTime, endTime, slotMinutes) {
...
doctorsRoutes.get('/:id/slots', async (c) => {
    ...
    const booked = await db
        ...
        .where(and(eq(doctorAppointments.doctorId, doctorId), eq(doctorAppointments.appointmentDate, date), eq(doctorAppointments.status, 'booked')));
...
doctorsRoutes.post('/:id/appointments', requireStudent, async (c) => {
    ...
    try {
        ...
        const matched = generateSlotsForDay(availability.startTime, availability.endTime, availability.slotMinutes).find((s) => s.startTime === startTime);
        if (!matched) {
